...
function showPage(p){
    ...
    const n=document.querySelector('[data-page="'+p+'"]');
    if(e)e.classList.add('active');
    if(n)n.classList.add('active');
    clearTimeout(_loadTimer);
    _loadTimer=setTimeout(()=>{
        if(p==='dashboard')loadSystemInfo();
        if(p==='services')loadServices();
        if(p==='startup')loadStartup();
        if(p==='disks')loadDisks();
    },100);
}
...
async function loadSystemInfo(){
    try{
        ...
        setEl('cpuValue',cpuPct+'%');setBar('cpuBar',cpuPct);
        setEl('cpuInfo',(d.cpu?.physical||'?')+' cores / '+(d.cpu?.logical||'?')+' threads');
        const ramPct=Math.round(d.memory?.percent||0);
        setEl('ramValue',ramPct+'%');setBar('ramBar',ramPct);
        setEl('ramInfo',(d.memory?.used_gb||0)+'GB / '+(d.memory?.total_gb||0)+'GB');
        if(d.disk&&d.disk.length){const dk=d.disk[0];setEl('diskValue',Math.round(dk.percent)+'%');setBar('diskBar',dk.percent);setEl('diskInfo',fmtB(dk.free)+' free / '+fmtB(dk.total))}
        const up=fmtSp(d.network?.speed_up),dn=fmtSp(d.network?.speed_down);
        setEl('netValue','<span class="speed-up">\u2191'+up+'</span> <span class="speed-down">\u2193'+dn+'</span>',true);
        setEl('netInfo','Sent: '+fmtB(d.network?.bytes_sent)+' | Recv: '+fmtB(d.network?.bytes_recv));
        const nb=document.getElementById('netBar');if(nb)nb.style.width=Math.min(100,Math.max(d.network?.speed_up||0,d.network?.speed_down||0,1)/1048576*100)+'%';
        if(d.gpu){setEl('gpuValue',(d.gpu.usage||0)+'%');setBar('gpuBar',d.gpu.usage||0);let gt=d.gpu.name||'N/A';if(d.gpu.temperature)gt+=' | '+d.gpu.temperature+'\u00b0C';if(d.gpu.memory_total)gt+=' | '+fmtB((d.gpu.memory_used||0)*1048576)+'/'+fmtB(d.gpu.memory_total);setEl('gpuName',gt)}
        if(d.system){setEl('uptimeValue',d.system.uptime);setEl('sysInfo',d.system.os+' | '+d.system.processor)}
        if(d.cpu?.temperature)setEl('tempValue',d.cpu.temperature+'\u00b0C');
        if(d.os){const b=document.getElementById('osBadge');if(b)b.textContent=d.os.os_name||'Unknown'}
    }catch(e){}
}
...
function fmtB(b){if(!b)return'--';const k=1024,s=['B','KB','MB','GB','TB'];const i=Math.floor(Math.log(b)/Math.log(k));return parseFloat((b/Math.pow(k,i)).toFixed(1))+' '+s[i]}
function fmtSp(bps){if(!bps||bps<1)return'0 B/s';const k=1024,s=['B/s','KB/s','MB/s','GB/s'];const i=Math.floor(Math.log(bps)/Math.log(k));return parseFloat((bps/Math.pow(k,i)).toFixed(1))+s[i]}
...
function startParticles(canvas){const ctx=canvas.getContext('2d');canvas.width=window.innerWidth;canvas.height=window.innerHeight;const ps=[];for(let i=0;i<40;i++)ps.push({x:Math.random()*canvas.width,y:Math.random()*canvas.height,vx:(Math.random()-.5)*.4,vy:(Math.random()-.5)*.4,r:Math.random()*2+1});function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);ps.forEach(p=>{p.x+=p.vx;p.y+=p.vy;if(p.x<0||p.x>canvas.width)p.vx*=-1;if(p.y<0||p.y>canvas.height)p.vy*=-1;ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle='rgba(124,58,237,0.3)';ctx.fill()});ps.forEach((a,i)=>{ps.slice(i+1).forEach(b=>{const d=Math.hypot(a.x-b.x,a.y-b.y);if(d<120){ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.strokeStyle='rgba(124,58,237,'+(1-d/120)*0.15+')';ctx.stroke()}})});particleAnim=requestAnimationFrame(draw)}draw()}
